...
class Slide {
    ...
    updateContentSize(force) {
        ...
        const width =
            Math.round(this.width * scaleMultiplier) ||
            this.pswp.viewportSize.x;
        const height =
            Math.round(this.height * scaleMultiplier) ||
            this.pswp.viewportSize.y;
    ...
    zoomTo(destZoomLevel, centerPoint, transitionDuration, ignoreBounds) {
        ...
        this.pan.x = this.calculateZoomToPanOffset(
            'x',
            centerPoint,
            prevZoomLevel
        );
        this.pan.y = this.calculateZoomToPanOffset(
            'y',
            centerPoint,
            prevZoomLevel
        );
...
class DragHandler {
    ...
    change() {
        ...
        if (
            ...
            if (
                !this.pswp.dispatch('verticalDrag', { panY }).defaultPrevented
            ) {
        ...
        if (mainScroll.isShifted()) {
            ...
            const mainScrollShiftDiff =
                mainScroll.x - mainScroll.getCurrSlideX();
            ...
            const currentSlideVisibilityRatio =
                mainScrollShiftDiff / this.pswp.viewportSize.x;
    ...
    _finishPanGestureForAxis(axis) {
        ...
        if (restoreBgOpacity) {
            ...
            const projectedVDragRatio =
                this._getVerticalDragRatio(projectedPosition);
        ...
        const dampingRatio =
            correctedPanPosition === projectedPosition ? 1 : 0.82;
    ...
    _panOrMoveMainScroll(axis) {
        ...
        if (
            ...
            if (newPan > bounds.min[axis] && isLeftToRight) {
                ...
                const wasAtMinPanPosition =
                    bounds.min[axis] <= this.startPan[axis];
                ...
                const wasAtMaxPanPosition =
                    this.startPan[axis] <= bounds.max[axis];
    ...
    _getVerticalDragRatio(panY) {
        return (
            (panY - (this.pswp.currSlide?.bounds.center.y ?? 0)) /
    ...
    change() {
        ...
        if (currZoomLevel < minZoomLevel) {
            if (
                ...
                if (
                    !pswp.dispatch('pinchClose', { bgOpacity }).defaultPrevented
                ) {
                ...
                currZoomLevel =
                    minZoomLevel -
                    (minZoomLevel - currZoomLevel) * LOWER_ZOOM_FRICTION;
            }
        } else if (currZoomLevel > maxZoomLevel) {
            // Apply the friction if zoom level is above the max
            currZoomLevel =
                maxZoomLevel +
                (currZoomLevel - maxZoomLevel) * UPPER_ZOOM_FRICTION;
        }
            ...
            onUpdate: (now) => {
                ...
                if (panNeedsChange || currZoomLevelNeedsChange) {
                    if (panNeedsChange) {
                        currSlide.pan.x =
                            initialPan.x +
                            (destinationPan.x - initialPan.x) * now;
                        currSlide.pan.y =
                            initialPan.y +
                            (destinationPan.y - initialPan.y) * now;
                    }
...
class TapHandler {
    ...
    _doClickOrTapAction(actionName, point, originalEvent) {
        ...
        if (
            pswp.dispatch(actionFullName, { point, originalEvent })
                .defaultPrevented
        ) {
            return;
        }
...
class Gestures {
    ...
    _bindEvents(pref, down, up, cancel) {
        ...
        events.add(
            window,
            pref + 'move',
            /** @type EventListener */ (this.onPointerMove.bind(this))
        );
        events.add(
            window,
            pref + up,
    ...
    onPointerDown(e) {
        ...
        const isMousePointer =
            e.type === 'mousedown' || e.pointerType === 'mouse';
        ...
        if (
            pswp.dispatch('pointerDown', { originalEvent: e }).defaultPrevented
        ) {
            return;
        }
    ...
    onPointerMove(e) {
        ...
        if (
            this.pswp.dispatch('pointerMove', { originalEvent: e })
                .defaultPrevented
        ) {
            return;
        }
    ...
    onPointerUp(e) {
        ...
        if (
            this.pswp.dispatch('pointerUp', { originalEvent: e })
                .defaultPrevented
        ) {
            return;
        }
    ...
    _finishTap(e) {
        ...
        const tapDelay = this.pswp.options.doubleTapAction
            ? DOUBLE_TAP_DELAY
            : 0;
        ...
        if (this._tapTimer) {
            ...
            if (
                getDistanceBetween(this._lastStartP1, this.startP1) <
                MIN_TAP_DISTANCE
            ) {
    ...
    _updatePoints(e, pointerType) {
        if (this._pointerEventEnabled) {
            ...
            const pointerIndex = this._ongoingPointers.findIndex(
                (ongoingPointer) => {
            ...
            if (pointerType === 'up' && pointerIndex > -1) {
                ...
                this._ongoingPointers.push(
                    this._convertEventPosToPoint(pointerEvent, { x: 0, y: 0 })
                );
    ...
    _calculateDragDirection() {
        if (this.pswp.mainScroll.isShifted()) {
            ...
            if (diff !== 0) {
                ...
                if (
                    Math.abs(
                        this.p1[axisToCheck] - this.startP1[axisToCheck]
    ...
    moveTo(x, dragging) {
        if (!this.pswp.canLoop() && dragging) {
            ...
            let newSlideIndexOffset =
                (this.slideWidth * this._currPositionIndex - x) /
                this.slideWidth;
            ...
            if (
                ...
                (newSlideIndexOffset >= this.pswp.getNumItems() - 1 &&
                    delta < 0)
            ) {
...
class SpringEaser {
    ...
    easeFrame(deltaPosition, deltaTime) {
        ...
        const naturalDumpingPow =
            Math.E **
            (-this._dampingRatio * this._naturalFrequency * deltaTime);

        if (this._dampingRatio === 1) {
            coeff = this.velocity + this._naturalFrequency * deltaPosition;

            displacement =
                (deltaPosition + coeff * deltaTime) * naturalDumpingPow;

            this.velocity =
                displacement * -this._naturalFrequency +
                coeff * naturalDumpingPow;
...
class ScrollWheel {
    ...
    constructor(pswp) {
        ...
        pswp.events.add(
            pswp.element,
            'wheel',
            /** @type EventListener */ (this._onWheel.bind(this))
        );
    }
    ...
    _onWheel(e) {
        ...
        if (
            this.pswp.dispatch('wheel', { originalEvent: e }).defaultPrevented
        ) {
            return;
        }
            ...
            if (!delayTimeout) {
                ...
                delayTimeout = setTimeout(() => {
                    setIndicatorVisibility(
                        Boolean(pswp.currSlide?.content.isLoading())
                    );
                    delayTimeout = null;
...
class Content {
    ...
    remove() {
        ...
        if (this.element && this.element.parentNode && !this.pictureElement) {
            this.element.remove();
        }
        if (this.pictureElement && this.pictureElement.parentNode) {
...
class ContentLoader {
    ...
    addToCache(content) {
        ...
        if (this._cachedItems.length > this.limit) {
            ...
            if (indexToRemove !== -1) {
                const removedItem = this._cachedItems.splice(
                    indexToRemove,
                    1
                )[0];
                removedItem.destroy();
...
class PhotoSwipeBase extends Eventable {
    ...
    getNumItems() {
        ...
        if (dataSource && 'length' in dataSource) {
            ...
            // query DOM elements
            if (!dataSource.items) {
                dataSource.items = this._getGalleryDOMElements(
                    dataSource.gallery
                );
            }
...
class Opener {
    ...
    close() {
        ...
        if (
            slide &&
            slide.currZoomLevel * slide.width >=
    ...
    _applyStartProps() {
        ...
        this._animateBgOpacity =
            ...
            this.pswp.options.bgOpacity > MIN_OPACITY;
    ...
    _start() {
        if (
            ...
            new Promise((resolve) => {
                ...
                let isDelaying = true;
                decodeImage(
                    /** @type {HTMLImageElement} */ (this._placeholder)
                ).finally(() => {
                    decoded = true;
                    if (!isDelaying) {
                        resolve(true);
    ...
    _setClosedStateZoomPan(animate) {
        ...
        if (
            ...
            const containerOnePanX =
                -viewportSize.x +
                (this._thumbBounds.x - innerRect.x) +
                innerRect.w;
            const containerOnePanY =
                -viewportSize.y +
                (this._thumbBounds.y - innerRect.y) +
                innerRect.h;
...
class PhotoSwipe extends PhotoSwipeBase {
    ...
    refreshSlideContent(slideIndex) {
        ...
        this.mainScroll.itemHolders.forEach((itemHolder, i) => {
            ...
            if (this.canLoop()) {
                potentialHolderIndex =
                    this.getLoopedIndex(potentialHolderIndex);
            }
